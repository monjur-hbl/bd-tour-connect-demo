// BD Tour Connect API - Cloudflare Pages Functions
interface Env {
  DB: D1Database;
}
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (path === '/agencies' && method === 'GET') {
      return handleGetAgencies(env);
    }

    if (path.match(/^\/agencies\/[\w-]+$/) && method === 'GET') {
    ...
    if (path.match(/^\/packages\/[\w-]+$/) && method === 'GET') {
    ...
    if (path.match(/^\/packages\/[\w-]+$/) && method === 'PUT') {
      const packageId = path.split('/').pop()!;
    ...
    if (path.match(/^\/bookings\/[\w-]+$/) && method === 'GET') {
    ...
    if (path.match(/^\/bookings\/[\w-]+$/) && method === 'PUT') {
      const bookingId = path.split('/').pop()!;
      return handleUpdateBooking(bookingId, request, env);
    }

    if (path === '/bookings/search' && method === 'GET') {
    ...
    if (path.match(/^\/agencies\/[\w-]+$/) && method === 'PUT') {
      const agencyId = path.split('/').pop()!;
      return handleUpdateAgency(agencyId, request, env);
    }
...
async function handleGetAgencies(env: Env): Promise<Response> {
...
async function handleGetAgency(agencyId: string, env: Env): Promise<Response> {
  ...
  return jsonResponse({
    agency: {
      ...
      loginBgImage: agency.login_bg_image,
      loginLogo: agency.login_logo,
      loginWelcomeText: agency.login_welcome_text,
...
async function handleGetBookings(request: Request, env: Env): Promise<Response> {
  ...
  if (packageId) { query += ' AND package_id = ?'; params.push(packageId); }
  ...
  const bookings = results.results.map((b: any) => ({
    id: b.id, bookingId: b.booking_id, packageId: b.package_id, agencyId: b.agency_id,
...
async function handleGetBooking(bookingId: string, env: Env): Promise<Response> {
  ...
  return jsonResponse({
    booking: {
      id: booking.id, bookingId: booking.booking_id, packageId: booking.package_id,
      agencyId: booking.agency_id, agentId: booking.agent_id, guestName: booking.guest_name,
...
async function handleCreateBooking(request: Request, env: Env): Promise<Response> {
  ...
  await env.DB.prepare('UPDATE packages SET available_seats = available_seats - ? WHERE id = ?').bind(passengerCount, body.packageId).run();
...
async function handleUpdateBooking(bookingId: string, request: Request, env: Env): Promise<Response> {
  ...
  await env.DB.prepare(`
    UPDATE bookings SET
      advance_paid = COALESCE(?, advance_paid), due_amount = COALESCE(?, due_amount),
      payment_status = COALESCE(?, payment_status), status = COALESCE(?, status),
      notes = COALESCE(?, notes), updated_at = datetime('now')
    WHERE id = ? OR booking_id = ?
  `).bind(
    body.advancePaid || null, body.dueAmount || null, paymentStatus || null,
    body.status || null, body.notes || null, bookingId, bookingId
  ).run();
...
async function handleSearchBookings(request: Request, env: Env): Promise<Response> {
  ...
  let sql = `SELECT * FROM bookings WHERE (booking_id LIKE ? OR guest_phone LIKE ? OR guest_name LIKE ?)`;
  const searchPattern = `%${query}%`;
  const params: any[] = [searchPattern, searchPattern, searchPattern];

  if (agencyId) { sql += ' AND agency_id = ?'; params.push(agencyId); }
  sql += ' ORDER BY created_at DESC LIMIT 20';

  const results = await env.DB.prepare(sql).bind(...params).all();

  const bookings = results.results.map((b: any) => ({
    id: b.id, bookingId: b.booking_id, packageId: b.package_id,
    guestName: b.guest_name, guestPhone: b.guest_phone, totalAmount: b.total_amount,
    dueAmount: b.due_amount, paymentStatus: b.payment_status, status: b.status, createdAt: b.created_at,
  }));
...
async function handleGetUsers(request: Request, env: Env): Promise<Response> {
  ...
  const users = results.results.map((u: any) => ({
    id: u.id, phone: u.phone, name: u.name, nameBn: u.name_bn, email: u.email,
    role: u.role, agencyId: u.agency_id, agentCode: u.agent_code,
...
async function handleCreateUser(request: Request, env: Env): Promise<Response> {
  ...
  if (existing) {
    return jsonResponse({ error: 'Phone number already exists' }, 400);
  }
  ...
  await env.DB.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    id, body.phone, body.password || 'agent123', body.name, body.nameBn || null,
    body.email || null, body.role, body.agencyId || null, agentCode || null,
...
async function handleUpdateUser(userId: string, request: Request, env: Env): Promise<Response> {
  const body = await request.json() as any;
  ...
  if (body.nameBn !== undefined) { updates.push('name_bn = ?'); params.push(body.nameBn); }
  ...
  if (body.password) { updates.push('password_hash = ?'); params.push(body.password); }
...
async function handleDeleteUser(userId: string, request: Request, env: Env): Promise<Response> {
...
async function getUserFromToken(request: Request, env: Env): Promise<any> {
  const token = request.headers.get('Authorization')?.replace('Bearer ', '');
...
async function handleUpdateProfile(request: Request, env: Env): Promise<Response> {
  ...
  return jsonResponse({
    ...
    user: {
      ...
      nameBn: updatedUser!.name_bn,
      email: updatedUser!.email,
      role: updatedUser!.role,
      agencyId: updatedUser!.agency_id,
      agentCode: updatedUser!.agent_code,
      permissions: updatedUser!.permissions ? JSON.parse(updatedUser!.permissions as string) : null,
    }
  });
}
...
async function handleChangePassword(request: Request, env: Env): Promise<Response> {
  ...
  ).bind(body.newPassword, user.id).run();

  return jsonResponse({ message: 'Password changed successfully' });
}

async function handleUpdateAgency(agencyId: string, request: Request, env: Env): Promise<Response> {
  const user = await getUserFromToken(request, env);
  if (!user) {
    return jsonResponse({ error: 'Unauthorized' }, 401);
  }

  // Only agency_admin of this agency or system_admin can update
  if (user.role !== 'system_admin' && (user.role !== 'agency_admin' || user.agency_id !== agencyId)) {
  ...
  if (body.addressBn !== undefined) { updates.push('address_bn = ?'); params.push(body.addressBn); }
  if (body.logo !== undefined) { updates.push('logo = ?'); params.push(body.logo); }
  if (body.primaryColor !== undefined) { updates.push('primary_color = ?'); params.push(body.primaryColor); }
  if (body.tagline !== undefined) { updates.push('tagline = ?'); params.push(body.tagline); }
  if (body.taglineBn !== undefined) { updates.push('tagline_bn = ?'); params.push(body.taglineBn); }
  // Login page customization fields
  if (body.loginBgImage !== undefined) { updates.push('login_bg_image = ?'); params.push(body.loginBgImage); }
  if (body.loginLogo !== undefined) { updates.push('login_logo = ?'); params.push(body.loginLogo); }
  if (body.loginWelcomeText !== undefined) { updates.push('login_welcome_text = ?'); params.push(body.loginWelcomeText); }
  if (body.loginWelcomeTextBn !== undefined) { updates.push('login_welcome_text_bn = ?'); params.push(body.loginWelcomeTextBn); }
  ...
  await env.DB.prepare(`UPDATE agencies SET ${updates.join(', ')} WHERE id = ?`).bind(...params).run();
...
async function handleGetStats(request: Request, env: Env): Promise<Response> {
  ...
  if (role === 'system_admin') {
    ...
    stats = {
      totalAgencies: agencyCount?.count || 0, totalAgents: userCount?.count || 0,
      totalBookings: bookingCount?.count || 0, todayBookings: todayBookings?.count || 0,
      totalRevenue: revenue?.total || 0, pendingAmount: pending?.total || 0,
    ...
    stats = {
      totalBookings: bookingCount?.count || 0, todayBookings: todayBookings?.count || 0,
      totalRevenue: revenue?.total || 0, pendingAmount: pending?.total || 0,
      activePackages: packageCount?.count || 0, totalAgents: agentCount?.count || 0,
    };
  }
  ...
  if (agentId) {
    ...
    stats = { ...stats, myBookings: bookingCount?.count || 0, myTodayBookings: todayBookings?.count || 0, myRevenue: revenue?.total || 0, myPending: pending?.total || 0 };
  }
